...
#define POSEIDON_MONGO_VALUE_VARIANT_TYPE_  \
  ...
  public:
    // Value constructors
    constexpr
    Mongo_Value(nullptr_t = nullptr)
      noexcept { }

    Mongo_Value(bool value)
      noexcept
      {
    ...
    Mongo_Value(const cow_string& utf8)
      noexcept
      {
    ...
    Mongo_Value&
    operator=(const cow_string& utf8)
      &
      {
        this->m_stor = utf8;
        return *this;
    ...
    Mongo_Value(const cow_bstring& bin)
      noexcept
      {
    ...
    Mongo_Value(const Mongo_Array& arr)
      noexcept
      {
    ...
    Mongo_Value(const Mongo_Document& doc)
      noexcept
      {
    ...
    Mongo_Value(const ::bson_oid_t& oid)
      noexcept
      {
...
inline
void
swap(Mongo_Value& lhs, Mongo_Value& rhs)
  noexcept
  { lhs.swap(rhs);  }
