...
struct Session_Table
  {
    ...
    ::std::unordered_map<const volatile WS_Server_Session*, Event_Queue> session_map;
  };
...
struct Final_Fiber final : Abstract_Fiber
  {
    ...
    Final_Fiber(const Easy_WS_Server::callback_type& callback,
                const shptr<Session_Table>& sessions, const volatile WS_Server_Session* refptr)
      :
    ...
    do_on_abstract_fiber_execute() override
      {
        for(;;) {
          ...
          try {
            ...
            POSEIDON_LOG_ERROR(("Unhandled exception thrown from easy HWS server: $1"), stdex);
