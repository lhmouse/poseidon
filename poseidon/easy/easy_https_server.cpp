...
struct Final_Fiber final : Abstract_Fiber
  {
    ...
    explicit
    Final_Fiber(const Shared_cb_args& cb, const volatile HTTPS_Server_Session* refptr)
...
struct Final_HTTPS_Server_Session final : HTTPS_Server_Session
  {
    ...
    explicit
    Final_HTTPS_Server_Session(unique_posix_fd&& fd, const Shared_cb_args& cb)
      : HTTPS_Server_Session(::std::move(fd), network_driver.default_server_ssl_ctx()), m_cb(cb)
      { }
    ...
    do_on_listen_new_client_opt(Socket_Address&& addr, unique_posix_fd&& fd) override
      {
        ...
        auto session = new_sh<Final_HTTPS_Server_Session>(::std::move(fd), this->m_cb);
        (void) addr;
