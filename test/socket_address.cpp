...
main()
  {
    ...
    POSEIDON_TEST_CHECK(addr.parse("1.2.128.255:12345", 17) == 17);
    ...
    ::memset(data, 0x66, size);
    POSEIDON_TEST_CHECK(addr.parse("3.2.128.255:", 12) == 0);
    POSEIDON_TEST_CHECK(::memcmp(data,
        "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00", 16) == 0);
    POSEIDON_TEST_CHECK(addr.port() == 0);

    ::memset(data, 0x66, size);
    POSEIDON_TEST_CHECK(addr.parse("3.2.128.255", 11) == 0);
    POSEIDON_TEST_CHECK(::memcmp(data,
        "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00", 16) == 0);
    POSEIDON_TEST_CHECK(addr.port() == 0);

    ::memset(data, 0x66, size);
    POSEIDON_TEST_CHECK(addr.parse("", 0) == 0);
    POSEIDON_TEST_CHECK(::memcmp(data,
        "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00", 16) == 0);
    POSEIDON_TEST_CHECK(addr.port() == 0);

    ::memset(data, 0x66, size);
    POSEIDON_TEST_CHECK(addr.parse("[fe80:1234:5678::90ab:cdef]:54321", 33) == 33);
    POSEIDON_TEST_CHECK(::memcmp(data,
        "\xfe\x80\x12\x34\x56\x78\x00\x00\x00\x00\x00\x00\x90\xAB\xCD\xEF", 16) == 0);
    POSEIDON_TEST_CHECK(addr.port() == 54321);

    ::memset(data, 0x66, size);
    POSEIDON_TEST_CHECK(addr.parse("[::]:7890", 9) == 9);
    POSEIDON_TEST_CHECK(::memcmp(data,
        "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00", 16) == 0);
    POSEIDON_TEST_CHECK(addr.port() == 7890);

    ::memset(data, 0x66, size);
    POSEIDON_TEST_CHECK(addr.parse("[fe80:1254:5678::90ab:cdef]:0", 29) == 29);
    POSEIDON_TEST_CHECK(::memcmp(data,
        "\xfe\x80\x12\x54\x56\x78\x00\x00\x00\x00\x00\x00\x90\xAB\xCD\xEF", 16) == 0);
    POSEIDON_TEST_CHECK(addr.port() == 0);

    Socket_Address cmp;
    cmp.parse("[fe80:1254:5678::90ab:cdef]:0", 29);
    POSEIDON_TEST_CHECK(addr.compare(cmp) == 0);
    POSEIDON_TEST_CHECK(cmp.compare(addr) == 0);
    POSEIDON_TEST_CHECK(addr == cmp);

    addr.parse("[fe80:1254:5678::90ab:cdef]:1", 29);
    POSEIDON_TEST_CHECK(addr.compare(cmp) > 0);
    POSEIDON_TEST_CHECK(cmp.compare(addr) < 0);
    POSEIDON_TEST_CHECK(addr > cmp);

    addr.parse("[fe80:1254:5678::90ab:cdee]:1", 29);
    POSEIDON_TEST_CHECK(addr.compare(cmp) < 0);
    POSEIDON_TEST_CHECK(cmp.compare(addr) > 0);
    POSEIDON_TEST_CHECK(addr < cmp);

    addr.parse("[fe80:1254:5678::90ab:ceee]:1", 29);
    POSEIDON_TEST_CHECK(addr.compare(cmp) > 0);
    POSEIDON_TEST_CHECK(cmp.compare(addr) < 0);
    POSEIDON_TEST_CHECK(addr > cmp);

    addr.parse("[fe70:1254:5678::90ab:cdee]:1", 29);
    POSEIDON_TEST_CHECK(addr.compare(cmp) < 0);
    POSEIDON_TEST_CHECK(cmp.compare(addr) > 0);
    POSEIDON_TEST_CHECK(addr < cmp);

    // classification
    POSEIDON_TEST_CHECK(addr.parse("0.0.0.0:42", 10) == 10);
    POSEIDON_TEST_CHECK(addr.classify() == ip_address_class_unspecified);

    POSEIDON_TEST_CHECK(addr.parse("10.6.123.84:80", 14) == 14);
    POSEIDON_TEST_CHECK(addr.classify() == ip_address_class_private);

    POSEIDON_TEST_CHECK(addr.parse("127.0.0.1:443", 13) == 13);
    POSEIDON_TEST_CHECK(addr.classify() == ip_address_class_loopback);

    POSEIDON_TEST_CHECK(addr.parse("127.0.0.5:445", 13) == 13);
    POSEIDON_TEST_CHECK(addr.classify() == ip_address_class_loopback);

    POSEIDON_TEST_CHECK(addr.parse("192.168.60.1:1023", 17) == 17);
    POSEIDON_TEST_CHECK(addr.classify() == ip_address_class_private);

    POSEIDON_TEST_CHECK(addr.parse("225.13.0.9:335", 14) == 14);
