...
print_rfc1123_partial(char* str)
  const noexcept
  {
    ...
    ts.tv_nsec = 0;
    ts.tv_sec = ::asteria::clamp_cast<::time_t>(ts.tv_sec, -2208988800, 253402300799);
    struct tm tm;
    ::gmtime_r(&(ts.tv_sec), &tm);

    size_t len = ::strftime_l(str, 30, "%a, %d %b %Y %T GMT", &tm, c_locale);
    ASTERIA_ASSERT(len == 29);
    return len;
  }
