...
struct Final_UDP_Socket final : UDP_Socket
  {
    ...
    explicit
    Final_UDP_Socket(thunk_function* cb_thunk, shared_ptrR<void> cb_obj, shared_ptrR<void> uniq)
      : UDP_Socket(),
...
do_abstract_fiber_on_work()
  {
    for(;;) {
      // The packet callback may stop this client, so we have to check for expiry
      // in every iteration.
      ...
      try {
        ...
        POSEIDON_LOG_ERROR((
            "Unhandled exception thrown from easy UDP client: $1"),
            stdex);
      }
    }
  }
...
start()
  {
    ...
    this->m_socket = ::std::make_shared<Final_UDP_Socket>(this->m_cb_thunk, this->m_cb_obj, this->m_uniq);
