...
void
event_callback(shptrR<WS_Server_Session> session, Abstract_Fiber& /*fiber*/,
               Easy_WS_Event event, linear_buffer&& data)
  {
    switch(event) {
      ...
      case easy_ws_text:
        ...
        session->ws_send(easy_ws_text, data);
        break;

      case easy_ws_binary:
        POSEIDON_LOG_ERROR(("example WS server received BINARY data: $1"), data);
        session->ws_send(easy_ws_binary, data);
        break;
...
start_server()
  {
    ...
    POSEIDON_LOG_ERROR(("example WS server started: bind = $1"),
