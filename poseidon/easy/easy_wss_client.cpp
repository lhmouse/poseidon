...
struct Final_WSS_Client_Session final : WSS_Client_Session
  {
    ...
    explicit
    Final_WSS_Client_Session(cow_stringR host, cow_stringR caddr, cow_stringR query,
          const Easy_WSS_Client::thunk_type& thunk, const shptr<Event_Queue>& queue)
      : WSS_Client_Session(host, caddr, query), m_thunk(thunk), m_wqueue(queue)
      { }
...
connect(chars_view addr)
  {
    ...
    auto session = new_sh<Final_WSS_Client_Session>(host_header, path, query, this->m_thunk, queue);
