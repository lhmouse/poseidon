...
thread_loop()
  {
    ...
    if(ASTERIA_UNEXPECT(!this->m_epoll_fd)) {
      // Safety is on.
      lock.unlock();
      ::sleep(1);
      return;
    ...
    ::epoll_event pev;
    ASTERIA_ASSERT(this->m_event_buf.size() >= sizeof(pev));
