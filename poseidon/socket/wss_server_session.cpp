...
wss_send_text(chars_proxy data)
  {
    ...
    // FIN + TEXT
    return this->do_wss_send_raw_data_frame(0b10000000 | 1, data);
  }
...
wss_send_binary(chars_proxy data)
  {
    ...
    // FIN + BINARY
    return this->do_wss_send_raw_data_frame(0b10000000 | 2, data);
  }
...
wss_ping(chars_proxy data)
  {
    ...
    // FIN + PING
    return this->do_wss_send_raw_frame(0b10000000 | 9, ctl_data);
  }
...
wss_shut_down(uint16_t status, chars_proxy reason) noexcept
  {
    ...
    try {
      // FIN + CLOSE
      succ = this->do_wss_send_raw_frame(0b10000000 | 8, ctl_data);
    }
    catch(exception& stdex) {
      POSEIDON_LOG_ERROR((
