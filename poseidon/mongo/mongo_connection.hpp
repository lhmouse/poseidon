...
class Mongo_Connection
  ...
  public:
    ...
    // Fetches a document from the reply to the last command. This function must
    ...
    // If the command has produced a reply which does not contain a cursor, the
    // reply is converted to a `Mongo_Document` literally and stored into `output`.
    // If the command has produced a reply which contains a cursor, a cursor is
    // created where subsequent documents will be fetched. If there is no more
    // document to fetch, `false` is returned.
    bool
