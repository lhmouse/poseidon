...
void
event_callback(shptrR<WSS_Client_Session> session, Abstract_Fiber& /*fiber*/, Easy_Socket_Event event, linear_buffer&& data)
  {
    switch(event) {
      case easy_socket_open:
        POSEIDON_LOG_WARN(("example WSS client established connection to `$1`: $2"), session->remote_address(), data);
        break;
...
timer_callback(shptrR<Abstract_Timer> /*timer*/, Abstract_Fiber& /*fiber*/, steady_time /*now*/)
  {
    ...
    switch(state) {
      ...
      case 1: {
        ...
        my_client.wss_send(websocket_text, data);
      ...
      case 2: {
        ...
        my_client.wss_send(websocket_bin, data);
      ...
      case 3: {
        ...
        my_client.wss_send(websocket_ping, data);
