...
main()
  {
    ...
    POSEIDON_TEST_CHECK(hval.as_datetime().as_seconds_since_epoch() == (seconds) 1469118411);
    ...
    POSEIDON_TEST_CHECK(hval.parse("http-token mumble", 17) == 10);
    POSEIDON_TEST_CHECK(hval.is_string());
    POSEIDON_TEST_CHECK(hval.as_string() == "http-token");

    POSEIDON_TEST_CHECK(hval.parse("\"escaped string\" mumble", 23) == 16);
    POSEIDON_TEST_CHECK(hval.is_string());
    POSEIDON_TEST_CHECK(hval.as_string() == "escaped string");

    POSEIDON_TEST_CHECK(hval.parse("1.5 mumble", 11) == 3);
    POSEIDON_TEST_CHECK(hval.is_number());
    POSEIDON_TEST_CHECK(hval.as_number() == 1.5);

    POSEIDON_TEST_CHECK(hval.parse("1.5z mumble", 11) == 4);
    POSEIDON_TEST_CHECK(hval.is_string());
    POSEIDON_TEST_CHECK(hval.as_string() == "1.5z");

    POSEIDON_TEST_CHECK(hval.parse("Thu, 21 Jul 2016 16:26:51 GMT mumble", 37) == 29);
    POSEIDON_TEST_CHECK(hval.is_datetime());
    POSEIDON_TEST_CHECK(hval.as_datetime().as_seconds_since_epoch() == (seconds) 1469118411);

    POSEIDON_TEST_CHECK(hval.parse("Thu, 21 Jul 2016 16:26:51| GMT mumble", 37) == 3);
    POSEIDON_TEST_CHECK(hval.is_string());
    POSEIDON_TEST_CHECK(hval.as_string() == "Thu");

    POSEIDON_TEST_CHECK(hval.parse(" mumble", 7) == 0);
  }
