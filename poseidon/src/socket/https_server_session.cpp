...
do_on_ssl_stream(linear_buffer& data, bool eof)
  {
    ...
    for(;;) {
      ...
      if(!this->m_req_parser.headers_complete()) {
        ...
        if(this->m_req_parser.error()) {
          data.clear();
          this->do_on_https_request_error(this->m_req_parser.headers().method == http_HEAD,
...
bool
HTTPS_Server_Session::
https_response(bool method_was_head, HTTP_S_Headers&& resp, chars_view data)
  {
    if(this->m_upgrade_ack.load())
      POSEIDON_THROW((
          "HTTPS connection switched to another protocol",
    ...
    return this->do_https_raw_response(resp, method_was_head ? "" : data);
  }
...
https_shut_down(HTTP_Status status)
  noexcept
  {
    ...
    try {
      ...
      HTTP_S_Headers resp;
      resp.status = status;
      resp.reason = ::asteria::sref(::http_status_str(static_cast<::http_status>(status)));
      ...
      succ = this->do_https_raw_response(resp, fmt.get_buffer());
    }
...
https_shut_down(int status)
  noexcept
  {
    ...
    return this->https_shut_down(real_status);
  }
