...
static Easy_HTTP_Server my_server(
  // callback
  *[](const shptr<HTTP_Server_Session>& session, Abstract_Fiber& fiber,
...
poseidon_module_main()
  {
    static constexpr char bind_addr[] = "[::]:3804";
    my_server.start(&bind_addr);
    POSEIDON_LOG_ERROR(("example HTTP server started: $1"), bind_addr);
  }
