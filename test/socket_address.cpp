...
main()
  {
    ...
    POSEIDON_TEST_CHECK(addr.parse("1.2.128.255:12345") == true);
    ...
    ::memset(data, 0x66, size);
    POSEIDON_TEST_CHECK(addr.parse("3.2.128.255:") == false);
    POSEIDON_TEST_CHECK(::memcmp(data,
    ...
    ::memset(data, 0x66, size);
    POSEIDON_TEST_CHECK(addr.parse("") == true);
    POSEIDON_TEST_CHECK(::memcmp(data,
        "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00", 16) == 0);
    POSEIDON_TEST_CHECK(addr.port() == 0);

    ::memset(data, 0x66, size);
    POSEIDON_TEST_CHECK(addr.parse("[fe80:1234:5678::90ab:cdef]:54321") == true);
    POSEIDON_TEST_CHECK(::memcmp(data,
        "\xfe\x80\x12\x34\x56\x78\x00\x00\x00\x00\x00\x00\x90\xAB\xCD\xEF", 16) == 0);
    POSEIDON_TEST_CHECK(addr.port() == 54321);

    ::memset(data, 0x66, size);
    POSEIDON_TEST_CHECK(addr.parse("[::]:7890") == true);
    POSEIDON_TEST_CHECK(::memcmp(data,
        "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00", 16) == 0);
    POSEIDON_TEST_CHECK(addr.port() == 7890);

    ::memset(data, 0x66, size);
    POSEIDON_TEST_CHECK(addr.parse("[fe80:1254:5678::90ab:cdef]:0") == true);
    POSEIDON_TEST_CHECK(::memcmp(data,
