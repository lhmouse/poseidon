    ...
    for(;;) {
      ...
      if(!this->m_parser.frame_payload_complete()) {
        ...
        if(this->m_parser.frame_header().opcode <= 7) {
          ...
          ASTERIA_ASSERT(is_any_of(opcode, { ws_TEXT, ws_BINARY }));
        ...
        if(this->m_parser.frame_header().fin)
          switch(this->m_parser.frame_header().opcode)
            ...
            case ws_CLOSE:
              {
                ASTERIA_ASSERT(this->m_closure_notified == false);
                this->m_closure_notified = true;
                data.clear();

                // Get the status code in big-endian order..
                char temp[2];
                ::asteria::store_be<uint16_t>(temp, ws_status_no_status_code);
                payload.getn(temp, 2);
                this->do_on_wss_close(::asteria::load_be<WS_Status>(temp), payload);
                return;
              }

            case ws_PING:
              POSEIDON_LOG_TRACE(("PING from `$1`: $2"), this->remote_address(), payload);
              this->do_wss_send_raw_frame(0x8A, payload);
...
wss_shut_down(WS_Status status, chars_view reason)
  noexcept
  {
    ...
    try {
      ...
      data.resize(2);
      ::asteria::store_be<uint16_t>(data.mut_data(), status);
